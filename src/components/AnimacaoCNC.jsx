...
export default function AnimacaoCNC() {
  ...
  const finaleOp = easeOut(win(t, 15.9, 16.8)) * (1 - win(t, 19.4, END))

  return (
    <figure className="an" ref={box}>
      <figcaption className="an__cap">
        <span className={`an__dot an__dot--${statusTone}`} aria-hidden="true" />
        <span>Agente de programação CNC</span>
      </figcaption>
      ...
      <div className="an__draw">
        <svg viewBox="0 0 540 250" className="an__svg" role="img"
          ...
          <g opacity={num((scanP > 0 && scanP < 1 ? 1 : 0) * (1 - win(t, 2.6, 2.85)))}
            ...
            <rect x="-42" y="46" width="42" height="150" fill="url(#anScan)" />
      ...
      <div className="an__foot">
        ...
        <p className="an__ready" style={{ opacity: num(easeOut(win(t, 15.35, 15.8))) }}>
          Programa pronto para revisão
        </p>
      </div>
      ...
      <div className="an__bar">
        <span className="an__stages" aria-hidden="true">
          {STAGE_LABEL.map((_, i) => (
            <i key={i} className={i === stageIdx ? 'is-now' : i < stageIdx ? 'is-past' : ''} />
          ))}
        </span>
      ...
      <div className="an__finale" style={{ opacity: num(finaleOp) }} aria-hidden={finaleOp < 0.5}>
        ...
        <span className="an__rule" style={{ width: `${(easeOut(win(t, 17.8, 18.6)) * 220).toFixed(0)}px` }} />
